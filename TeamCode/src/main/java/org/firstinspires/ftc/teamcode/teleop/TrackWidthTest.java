...
@TeleOp(name = "TrackWidthTest")
public class TrackWidthTest extends CommandOpMode {
    ...
    public void initOpMode() {
        ...
        scheduler.schedule(FnCommand.repeat(t -> {
            ...
            if (Double.isNaN(hFinal)) {
                telemetry.addLine("Turn 10 times and press A to finish");
                telemetry.addData("X", p.x());
                telemetry.addData("Y", p.y());
                telemetry.addData("Heading", p.h());
            } else {
                telemetry.addData("Raw heading", hFinal);
                telemetry.addData("Predicted trackwidth", 20 * PI * MecanumDrive.parDist / hFinal);
            }
        }));
    }
}