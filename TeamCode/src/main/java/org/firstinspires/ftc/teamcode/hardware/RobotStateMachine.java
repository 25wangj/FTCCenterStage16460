...
public class RobotStateMachine {
    ...
    public static StateMachine<robotStates> get(CommandOpMode opMode, Robot robot, robotStates state) {
        ...
        builder.addTransition(robotStates.INTAKE_CLOSED, robotStates.EJECT_CLOSED, FnCommand.once(t -> {
                robot.intake.setRoller(rollerUp);
                robot.intake.setPower(intakeEject);}, subsystems));
        builder.addTransition(robotStates.EJECT_CLOSED, robotStates.INTAKE_CLOSED, FnCommand.once(t -> {
                robot.intake.setRoller(rollerDown);
                robot.intake.setPower(intakeClosed);}, subsystems));
        builder.addTransition(robotStates.INTAKE_OPEN, robotStates.EJECT_OPEN, FnCommand.once(t -> {
                robot.intake.setRoller(rollerUp);
                robot.intake.setPower(intakeEject);}, subsystems));
        builder.addTransition(robotStates.EJECT_OPEN, robotStates.INTAKE_OPEN, FnCommand.once(t -> {
                robot.intake.setRoller(rollerDown);
        ...
        builder.addTransition(robotStates.RETRACT, robotStates.DEPOSIT, new WaitCommand(t ->
                robot.lift.setClaw(clawClosed), 0.25, subsystems));
        builder.addTransition(robotStates.RETRACT, robotStates.INTAKE_CLOSED, new SeqCommand(
        ...
        builder.addTransition(robotStates.RETRACT, robotStates.INTAKE_OPEN, new SeqCommand(
                new ParCommand(robot.lift.goBack(), new WaitCommand(t -> {
                    robot.intake.setGate(gateOpen);
                    robot.intake.setRoller(rollerDown);}, 0.25, robot.intake)),
                FnCommand.once(t -> robot.intake.setPower(intakeOpen), subsystems)));
